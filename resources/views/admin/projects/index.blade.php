    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-wrap items-center gap-4">
                <!-- Filter Bar -->
                <div class="dark:bg-gray-800 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 w-full md:w-auto">
                    <form method="GET" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 m-0">
                        <div class="flex items-center justify-between sm:justify-start gap-2">
                            ...
                            <select name="status" onchange="this.form.submit()" class="text-sm rounded-lg border-gray-200 focus:ring-indigo-500 focus:border-indigo-500 py-1.5 pl-3 pr-10 border-0 shadow-sm w-full sm:w-auto text-center sm:text-left">
                        ...
                        <div class="flex items-center justify-between sm:justify-start gap-2 border-l-0 sm:border-l border-gray-200 pl-0 sm:pl-4">
                ...
                <!-- Bulk Actions -->
                <div class="dark:bg-gray-800 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 w-full md:w-auto">
                    <form id="bulkActionForm" method="POST" action="{{ route('admin.projects.bulk') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center m-0 gap-2" onsubmit="return confirm('Are you sure you want to perform this bulk action?');">
                        @csrf
                        <label class="text-xs font-white uppercase text-gray-400 whitespace-nowrap">Bulk Action:</label>
                        <select name="action" id="bulkActionSelect" required class="text-sm rounded-lg border-gray-200 focus:ring-indigo-500 focus:border-indigo-500 py-1.5 pl-3 pr-8 w-full sm:w-auto text-center sm:text-left" onchange="document.getElementById('bulkAdviserSelect').classList.toggle('hidden', this.value !== 'reassign'); document.getElementById('bulkAdviserSelect').disabled = this.value !== 'reassign';">
                        ...
                        <select name="adviser_id" id="bulkAdviserSelect" class="text-sm rounded-lg border-gray-200 focus:ring-indigo-500 focus:border-indigo-500 py-1.5 pl-3 pr-8 hidden w-full sm:w-auto text-center sm:text-left" disabled required>
                            <option value="">Select Adviser...</option>
                            @foreach($advisers as $adviser)
                                <option value="{{ $adviser->id }}">{{ $adviser->name }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="px-4 py-2 sm:py-1.5 bg-indigo-600 text-white text-[10px] font-black uppercase rounded shadow hover:bg-indigo-700 transition w-full sm:w-auto">Apply</button>
                    </form>
                </div>
