<x-app-layout>
    ...
    <div class="py-8 bg-gray-50 dark:bg-slate-950 transition-colors">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- Ticket Header Card --}}
            <div
                class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm dark:shadow-sm border border-gray-200 dark:border-white/5 overflow-hidden transition-colors">
                <div class="p-6">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0" x-data="{ confirmDelete: false }">
                            ...
                            <div x-show="confirmDelete" x-cloak
                                ...
                                <div
                                    ...
                                    <h4 class="text-lg font-black text-gray-900 dark:text-white mb-2">Delete Ticket?
                                    </h4>
                                    <p class="text-sm text-gray-500 dark:text-slate-500 mb-6">This action cannot be
                                        undone. The ticket will
                    ...
                    {{-- Submitter Info --}}
                    <div
                        class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-slate-950/50 rounded-xl border border-gray-200 dark:border-white/5">
            ...
            {{-- Ticket Message --}}
            <div
                class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-white/5 overflow-hidden transition-colors">
                <div class="p-5 border-b border-gray-200 dark:border-white/5 bg-gray-100 dark:bg-slate-800">
                    <h4 class="font-black text-sm text-gray-900 dark:text-white flex items-center gap-2">
                        <svg class="w-4 h-4 text-blue-500 dark:text-indigo-500" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="p-6">
                    <p class="text-sm text-gray-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{{ trim($ticket->message) }}</p>
                    ...
                    @if($ticket->attachment_path)
                        <div class="mt-6 pt-6 border-t border-gray-100 dark:border-white/5">
                            ...
                            <div
                                class="rounded-2xl overflow-hidden border border-gray-200 dark:border-white/10 shadow-lg inline-block">
                                <a href="{{ asset('storage/' . $ticket->attachment_path) }}" target="_blank">
                                    <img src="{{ asset('storage/' . $ticket->attachment_path) }}"
                                        class="max-w-md w-full h-auto hover:opacity-90 transition-opacity cursor-zoom-in">
                                </a>
                            </div>
                        </div>
                    @endif
                ...
                <div
                    ...
                    @if($ticket->is_security_alert)
                        <div class="p-8 text-center bg-rose-50 dark:bg-rose-950/10">
                            <div
                                class="w-16 h-16 rounded-full bg-rose-100 dark:bg-red-900/20 flex items-center justify-center mx-auto mb-4">
                            ...
                            <p class="text-sm text-gray-500 dark:text-slate-400 mb-8 max-w-md mx-auto">This is an automated
                                alert. Once you have
                    ...
                    @else
                        @if($ticket->user_id || $ticket->email)
                            <div class="p-5 border-b border-blue-200 dark:border-indigo-500/20 bg-blue-50 dark:bg-indigo-900/20">
                                ...
                                <p class="text-xs text-blue-600/60 dark:text-indigo-400/60 mt-0.5">The user will see this message in
                                    the System Support panel. The ticket auto-deletes after 3 days.</p>
                            </div>
                            <div class="p-6">
                                <form method="POST" action="{{ route('admin.support.status', $ticket) }}">
                                    @csrf
                        ...
                        @else
                            <div class="p-5 border-b border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-900/20">
                                <h4 class="font-black text-sm text-emerald-600 dark:text-emerald-400 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-emerald-500 dark:text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                    Acknowledge Feedback & Resolve
                                </h4>
                                <p class="text-xs text-emerald-600/60 dark:text-emerald-400/60 mt-0.5">This ticket was submitted anonymously. No reply is needed.</p>
                            </div>
                            <div class="p-6">
                                <form method="POST" action="{{ route('admin.support.status', $ticket) }}">
                                    @csrf
            ...
            @else
                <div
                    class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-emerald-200 dark:border-emerald-500/20 overflow-hidden transition-colors">
                    <div
                        class="p-5 border-b border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-900/20">
                        <h4 class="font-black text-sm text-emerald-600 dark:text-emerald-400 flex items-center gap-2">
                        ...
                        @if($ticket->expires_at)
                            <p class="text-xs text-emerald-600/60 dark:text-emerald-400/60 mt-0.5">This ticket will be
                                automatically deleted
                                {{ $ticket->expires_at->diffForHumans() }}.
                            </p>
                        @endif
                    </div>
                    <div class="p-6">
                        @if($ticket->admin_reply)
                            <p class="text-sm text-gray-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{{ trim($ticket->admin_reply) }}</p>
                        @else
