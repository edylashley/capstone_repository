        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-white/5 overflow-hidden transition-colors"
                ...
                <div x-show="selected.length > 0" x-transition:enter="transition ease-out duration-300"
                    ...
                    class="p-4 bg-blue-600 border-b border-blue-500 flex items-center justify-between">
                    <p class="text-xs font-black text-white uppercase tracking-widest">
                        <span x-text="selected.length"></span> Tickets Selected
                    </p>
                    <form action="{{ route('admin.support.bulk-delete') }}" method="POST"
                        onsubmit="return confirm('Are you sure you want to delete these ' + selected.length + ' tickets? This cannot be undone.')">
                        @csrf
                        <template x-for="id in selected" :key="id">
                            <input type="hidden" name="ids[]" :value="id">
                        </template>
                        <button type="submit"
                            class="px-4 py-2 bg-red-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-700 transition-all shadow-lg flex items-center gap-2">
                ...
                @else
                        ...
                        @foreach($tickets as $ticket)
                            <div
                                ...
                                <div class="flex-shrink-0 cursor-pointer"
                                    onclick="window.location.href='{{ route('admin.support.show', $ticket) }}'">
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="hidden sm:block flex-shrink-0 text-right">
                                        <p class="text-xs font-bold text-gray-400">{{ $ticket->created_at->diffForHumans() }}
                                        </p>
                                    ...
                                    <a href="{{ route('admin.support.show', $ticket) }}" class="flex-shrink-0">
                                        <svg class="w-5 h-5 text-gray-400 dark:text-slate-700 group-hover:text-gray-900 dark:group-hover:text-white transition-all transform group-hover:translate-x-1 duration-300"
                    ...
                    {{-- Pagination --}}
                    <div class="p-5 border-t border-gray-200 dark:border-white/5 transition-colors">
                        {{ $tickets->links() }}
                    </div>
                @endif
            </div>
