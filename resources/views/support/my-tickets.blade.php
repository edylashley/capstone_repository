        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
                ...
                <div class="flex items-center gap-3">
                    <button @click="supportOpen = true" class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 border border-indigo-500/30">
            ...
            @if($tickets->isEmpty())
                <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-white/5 p-12 text-center transition-colors">
                    ...
                    <p class="text-sm text-gray-500 dark:text-slate-500 max-w-sm mx-auto">You haven't submitted any support tickets. Use the <span class="font-bold text-blue-500 dark:text-indigo-400">Create New Ticket</span> button above to submit one.</p>
                </div>
            @else
                <div class="space-y-4">
                            ...
                            <div class="p-5">
                                <div class="flex items-start justify-between gap-3 mb-3">
                                    ...
                                    <div class="flex items-center gap-3 flex-shrink-0">
                                        <span class="text-xs font-bold text-gray-400 dark:text-gray-300">#{{ $ticket->id }}</span>
                                        <button type="button" 
                                            @click="$store.deleteModal.show('Delete Ticket', 'Are you sure you want to permanently delete this support ticket? This action cannot be undone.', 'delete-ticket-{{ $ticket->id }}', false)"
                                            class="p-2 text-gray-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-all"
                                            title="Delete Ticket">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                            </svg>
                                        </button>
                                        <form id="delete-ticket-{{ $ticket->id }}" action="{{ route('support.user-destroy', $ticket) }}" method="POST" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </div>
