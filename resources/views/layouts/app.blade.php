    ...
    <head>
        ...
        <style>
            @media (min-width: 1024px) {
                ...
                .sidebar-collapsed-offset { margin-left: 0 !important; }
            }
        </style>
        @stack('styles')
    </head>
    <body class="font-sans antialiased bg-slate-100 text-slate-900" x-data="{ sidebarOpen: false, sidebarCollapsed: false }">
                    ...
                    @if(!request()->is('/'))
                        <div class="lg:hidden bg-indigo-700 text-white p-4 flex items-center justify-between shadow-md">
                            <div class="flex items-center gap-2">
                                <img src="{{ asset('images/system-logo.jpg') }}" class="w-8 h-8 object-contain rounded-full" alt="NORSU Capstone Repository">
                ...
                @if(isset($header) || View::hasSection('header'))
                    <header class="bg-gray-800 shadow-sm border-b border-gray-700">
                        <div class="py-4 md:py-6 px-4 md:px-8 text-white flex items-center gap-4">
                            ...
                            <button @click="sidebarCollapsed = false" x-show="sidebarCollapsed" x-cloak class="hidden lg:block p-2 rounded-lg bg-gray-700 hover:bg-gray-600 transition-colors focus:outline-none" title="Show Sidebar">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                            </button>
                            <div class="flex-1">
                                @if(isset($header))
        ...
        <div x-data="{ supportOpen: false }" class="relative z-[9990]">
            ...
            <div 
                ...
                x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0"
                class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-[9991]"
            ...
            >
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden pointer-events-auto flex flex-col max-h-full">
                    {{-- Header --}}
                    <div class="bg-gradient-to-r from-indigo-600 to-indigo-800 p-6 text-white flex justify-between items-center shrink-0">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-indigo-200 text-xs font-semibold uppercase tracking-wider">Report an Issue or Contact Admin</p>
                            </div>
                        </div>
                        <button @click="supportOpen = false" class="text-white/60 hover:text-white transition-colors bg-white/10 hover:bg-white/20 rounded-lg p-2">
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar">
                            ...
                            @if($latestReply)
                                <div class="mb-6 bg-green-50 rounded-xl border border-green-200 overflow-hidden relative">
                                    ...
                                    <div class="p-4 border-b border-green-200/50">
                                        ...
                                        <p class="text-xs text-green-600 font-semibold mb-2">Regarding: {{ $latestReply->subject }}</p>
                                        <div class="bg-white/60 p-3 rounded-lg">
                                            <p class="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{{ $latestReply->admin_reply }}</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
                        ...
                        <form action="{{ route('support.store') }}" method="POST" id="support-form" class="space-y-5">
                            @csrf
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Issue Category <span class="text-red-500">*</span></label>
                                <select name="category" required class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm text-sm">
                            ...
                            <div>
                                ...
                                <input type="text" name="subject" required placeholder="Brief title of your issue" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm text-sm">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Details <span class="text-red-500">*</span></label>
                                <textarea name="message" rows="4" required placeholder="Please provide specific details so the admin can assist you efficiently..." class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm text-sm resize-none"></textarea>
                            </div>

                            @if(!auth()->check())
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-1">Your Email <span class="text-red-500">*</span></label>
                                <input type="email" name="email" required placeholder="We need this to reply to you" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm text-sm">
                            </div>
                            @endif
                            ...
                            <div class="pt-2">
                                ...
                                <p class="text-[10px] text-center text-gray-400 mt-3 font-semibold uppercase tracking-wider">
        ...
        >
            ...
            @if($adminOpenTickets > 0)
                <span class="absolute -top-1 -right-1 w-6 h-6 bg-red-500 text-white text-[10px] font-black rounded-full flex items-center justify-center shadow-lg animate-pulse ring-2 ring-white">{{ $adminOpenTickets }}</span>
            @endif
        </a>
        @endif
    </body>
