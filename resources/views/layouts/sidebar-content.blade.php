...
<nav class="flex-1 flex flex-col gap-1 py-4 px-3 overflow-y-auto custom-scrollbar no-scrollbar">
    ...
    @if(auth()->user()->isAdmin())
        ...
        <a href="{{ route('admin.archive.index') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 transition-all duration-200 {{ request()->routeIs('admin.archive.*') ? 'bg-slate-800 shadow-md text-white' : 'text-slate-400' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
